...
class ProgramDatabase:
    ...
    def add(
        ...
        Returns:
            Program ID
        """
        # Skip programs with invalid metrics to avoid polluting the archive.
        if not is_valid_metrics(program.metrics):
            logger.info(
                "Skipping program %s due to invalid metrics (combined_score missing or error).",
                program.id,
            )
            return program.id
        ...
        for dim in self.config.feature_dimensions:
            ...
            # PRIORITY 2: Fall back to built-in features if not in metrics
                ...
                if not program.metrics:
                    ...
                    score_value = self._get_score_feature_value(program.metrics)
                    # Update stats and scale
                    self._update_feature_stats("score", score_value)
                    scaled_value = self._scale_feature_value("score", score_value)
    ...
    def _get_score_feature_value(self, metrics: Dict[str, Any]) -> float:
        """
        Compute the MAP-Elites "score" feature value with optional c5-based transforms.
        """
        transform = getattr(self.config, "score_feature_transform", None)
        baseline = getattr(self.config, "score_feature_baseline", None)
        c5_bound = metrics.get("c5_bound")

        if transform and isinstance(c5_bound, (int, float)) and c5_bound > 0:
            if transform == "baseline_ratio":
                if baseline and baseline > 0:
                    return float(baseline) / float(c5_bound) - 1.0
            elif transform == "neg_log":
                return -math.log(float(c5_bound))
            elif transform == "inverse":
                return 1.0 / float(c5_bound)
    ...
    def log_island_status(self) -> None:
        ...
        for stat in stats:
            ...
            logger.info(
                ...
                f"best={stat['best_score']:.12f}, avg={stat['average_score']:.12f}, "
    ...
    def log_prompt(
        ...
        # Persist prompt immediately so exports can see recent iterations
        try:
            if self.config.db_path:
                prompts_dir = os.path.join(self.config.db_path, "prompts")
                os.makedirs(prompts_dir, exist_ok=True)
                file_path = os.path.join(prompts_dir, f"{program_id}_{template_key}.json")
                payload = {
            ...
            logger.warning(f"Failed to write prompt log for {program_id}: {e}")
