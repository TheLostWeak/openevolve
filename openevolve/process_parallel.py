...
from openevolve.utils.metrics_utils import safe_numeric_average, get_fitness_score, is_valid_metrics
...
class SerializableResult:
    ...
    meta_prompt_candidate: Optional[str] = None
...
def _run_iteration_worker(
    ...
    try:
        ...
        meta_prompt_id = None
        meta_prompt_text = ""
        if _worker_config.prompt.use_meta_prompting:
            meta_prompt_path = resolve_meta_prompt_path(
                _worker_config.prompt.meta_prompt_db_path, _worker_config.database.db_path
            )
            meta_store = MetaPromptStore(
                meta_prompt_path, max_entries=_worker_config.prompt.meta_prompt_max_entries
            )
            rng = random.Random()
            if _worker_config.random_seed is not None:
                rng.seed(_worker_config.random_seed + iteration)
            if rng.random() < _worker_config.prompt.meta_prompt_weight:
                entry = meta_store.sample(
                    rng,
                    use_softmax=_worker_config.prompt.meta_prompt_use_softmax,
                    temperature=_worker_config.prompt.meta_prompt_temperature,
                )
                if entry:
        ...
        from openevolve.iteration import get_reasoning_message_history
        ...
        if isinstance(llm_response_obj, dict):
            ...
            if reasoning_details is not None:
                ...
                # Import here to avoid circular dependency
                from openevolve.iteration import _reasoning_message_history
        ...
        meta_prompt_candidate = None
        if is_valid_metrics(child_metrics):
            if (
                _worker_config.prompt.use_meta_prompting
                and _worker_config.prompt.meta_prompt_update_every > 0
                and iteration % _worker_config.prompt.meta_prompt_update_every == 0
            ):
                system_message, user_message = build_meta_prompt_request(
                    {
                        ...
                        "error": child_metrics.get("error") if child_metrics else "",
                    },
                    max_chars=_worker_config.prompt.meta_prompt_max_chars,
                )
                ...
                meta_prompt_candidate = parse_meta_prompt_response(
                    meta_response, _worker_config.prompt.meta_prompt_max_chars
                )
        ...
        return SerializableResult(
            ...
            meta_prompt_text=meta_prompt_text.strip() if meta_prompt_text else None,
            meta_prompt_candidate=meta_prompt_candidate,
        )
...
class ProcessParallelController:
    ...
    def _serialize_config(self, config: Config) -> dict:
        ...
        prompt_dict = asdict(config.prompt)
        if prompt_dict.get("meta_prompt_db_path"):
            prompt_dict["meta_prompt_db_path"] = os.path.abspath(
                prompt_dict["meta_prompt_db_path"]
            )
    ...
    def _update_meta_prompt_store(
        self, result: SerializableResult, child_program: Program
    ) -> None:
        if not self.config.prompt.use_meta_prompting:
            return
        if not is_valid_metrics(child_program.metrics):
            return
        meta_prompt_path = resolve_meta_prompt_path(
            self.config.prompt.meta_prompt_db_path, self.config.database.db_path
        )
        store = MetaPromptStore(
            meta_prompt_path, max_entries=self.config.prompt.meta_prompt_max_entries
        )

        child_fitness = get_fitness_score(
            child_program.metrics, self.database.config.feature_dimensions
        )

        if result.meta_prompt_id:
            parent_program = self.database.get(result.parent_id) if result.parent_id else None
            if parent_program:
                parent_fitness = get_fitness_score(
                    parent_program.metrics, self.database.config.feature_dimensions
                )
                store.update_score(result.meta_prompt_id, child_fitness - parent_fitness)
                store.record_use(result.meta_prompt_id)

        if result.meta_prompt_candidate:
            store.add(result.meta_prompt_candidate, child_fitness)
        ...
        while (
            ...
            try:
                ...
                if result.error:
                    ...
                    # Update meta prompt evolution store
                    if self.config.prompt.use_meta_prompting:
                        self._update_meta_prompt_store(result, child_program)
                    ...
                    if child_program.metrics:
                        metrics_str = ", ".join(
                            [
                                f"{k}={v:.12f}" if isinstance(v, (int, float)) else f"{k}={v}"
                        ...
                        if (
                            ...
                            logger.warning(
                                f"NOTE: No 'combined_score' metric found in evaluation results. "
