    ...
    async def evaluate_program(
        ...
        for attempt in range(self.config.max_retries + 1):
            ...
            try:
                ...
                # Ensure combined_score exists to keep downstream logic consistent.
                if "combined_score" not in eval_result.metrics:
                    eval_result.metrics["combined_score"] = 0.0
                    if eval_result.metrics.get("error") in (None, "", 0, 0.0):
                        eval_result.metrics["error"] = "missing combined_score"
