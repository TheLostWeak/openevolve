...
class OpenEvolve:
    ...
    def _save_best_program(self, program: Optional[Program] = None) -> None:
        ...
        # Execute best program and save output (e.g., for math problem solutions)
        try:
            output_file = os.path.join(best_dir, "best_program_output.txt")
            logger.info(f"Executing best program to capture output...")
            
            import subprocess
            import sys
            
            # Execute the best program and capture output
            ...
            with open(output_file, "w") as f:
                f.write("=== BEST PROGRAM EXECUTION OUTPUT ===\n")
                f.write(f"Program ID: {program.id}\n")
                f.write(f"Metrics: {program.metrics}\n")
                f.write(f"\n=== STDOUT ===\n")
            ...
            logger.info(f"Saved best program execution output to {output_file}")
        except Exception as e:
            logger.warning(f"Failed to execute best program and capture output: {e}")
