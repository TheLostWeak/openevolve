...
def format_metrics_safe(metrics: Dict[str, Any]) -> str:
    ...
    for name, value in metrics.items():
        ...
        if isinstance(value, (int, float)):
            try:
                # Only apply float formatting to numeric values (higher precision for logging)
                formatted_parts.append(f"{name}={value:.12f}")
        ...
        if metric in parent_metrics:
            ...
            if isinstance(child_value, (int, float)) and isinstance(parent_value, (int, float)):
                try:
                    ...
                    improvement_parts.append(f"{metric}={diff:+.12f}")
                except (ValueError, TypeError):
