...
def is_valid_metrics(metrics: Dict[str, Any]) -> bool:
    """
    Determine whether metrics are valid for evolution/logging.

    Rules:
    - combined_score must exist and be numeric
    - error must be empty/zero if present
    """
    if not metrics:
        return False
