...
class OpenAILLM(LLMInterface):
    ...
    def __init__(
        ...
        self.extra_body = getattr(model_cfg, "extra_body", None) or {}
    ...
    async def generate_with_context(
        ...
        If kwargs contains return_message=True, return a dict with content and reasoning_details
        (and the raw message) so callers can chain reasoning across calls. Otherwise return
        just the assistant content string (default behavior).
        """
        ...
        # Merge provider-specific extra_body (e.g., OpenRouter reasoning)
        merged_extra_body = {}
        if self.extra_body:
        ...
        return_message = kwargs.get("return_message", False)

        for attempt in range(retries + 1):
            try:
                response = await asyncio.wait_for(
                    self._call_api(params, return_message=return_message), timeout=timeout
                )
                return response
    ...
    async def _call_api(self, params: Dict[str, Any], return_message: bool = False) -> Any:
        ...
        logger.info(f"_call_api: return_message={return_message}")

        # If caller wants full message for reasoning chaining, return dict
        # containing content and reasoning_details (if present).
        if return_message:
            reasoning_details = getattr(message, "reasoning_details", None)
            logger.info(
                f"_call_api: returning dict with reasoning_details={reasoning_details is not None}"
            )
            return {
